...
GLT_API GLTtext* gltCreateText(void)
{
	...
	glad_glGenVertexArrays(1, &text->_vao);
	glad_glGenBuffers(1, &text->_vbo);
	...
	glad_glBindVertexArray(text->_vao);

	glad_glBindBuffer(GL_ARRAY_BUFFER, text->_vbo);

	glad_glEnableVertexAttribArray(_GLT_TEXT2D_POSITION_LOCATION);
	glad_glVertexAttribPointer(_GLT_TEXT2D_POSITION_LOCATION, _GLT_TEXT2D_POSITION_SIZE, GL_FLOAT, GL_FALSE, (_GLT_TEXT2D_VERTEX_SIZE * sizeof(GLfloat)), (const void*)(_GLT_TEXT2D_POSITION_OFFSET * sizeof(GLfloat)));

	glad_glEnableVertexAttribArray(_GLT_TEXT2D_TEXCOORD_LOCATION);
	glad_glVertexAttribPointer(_GLT_TEXT2D_TEXCOORD_LOCATION, _GLT_TEXT2D_TEXCOORD_SIZE, GL_FLOAT, GL_FALSE, (_GLT_TEXT2D_VERTEX_SIZE * sizeof(GLfloat)), (const void*)(_GLT_TEXT2D_TEXCOORD_OFFSET * sizeof(GLfloat)));
...
GLT_API void gltDeleteText(GLTtext *text)
{
	...
	if (text->_vao)
	{
		glad_glDeleteVertexArrays(1, &text->_vao);
		text->_vao = GLT_NULL_HANDLE;
	}

	if (text->_vbo)
	{
		glad_glDeleteBuffers(1, &text->_vbo);
...
GLT_API void gltBeginDraw()
{
	glad_glUseProgram(_gltText2DShader);

	glad_glActiveTexture(GL_TEXTURE0);
	glad_glBindTexture(GL_TEXTURE_2D, _gltText2DFontTexture);
}
...
#define _gltDrawText() \
	glad_glUniformMatrix4fv(_gltText2DShaderMVPUniformLocation, 1, GL_FALSE, mvp); \
	\
	glad_glBindVertexArray(text->_vao); \
	glad_glDrawArrays(GL_TRIANGLES, 0, text->vertexCount);
...
GLT_API void gltColor(GLfloat r, GLfloat g, GLfloat b, GLfloat a)
{
	glad_glUniform4f(_gltText2DShaderColorUniformLocation, r, g, b, a);
}

GLT_API void gltGetColor(GLfloat *r, GLfloat *g, GLfloat *b, GLfloat *a)
{
	GLfloat color[4];
	glad_glGetUniformfv(_gltText2DShader, _gltText2DShaderColorUniformLocation, color);
...
{
	GLint dimensions[4];
	glad_glGetIntegerv(GL_VIEWPORT, dimensions);
...
GLT_API void _gltUpdateBuffers(GLTtext *text)
{
	...
	glad_glBindBuffer(GL_ARRAY_BUFFER, text->_vbo);
	glad_glBufferData(GL_ARRAY_BUFFER, vertexCount * _GLT_TEXT2D_VERTEX_SIZE * sizeof(GLfloat), vertices, GL_DYNAMIC_DRAW);
...
GLT_API void gltTerminate(void)
{
	...
	{
		glad_glDeleteProgram(_gltText2DShader);
		_gltText2DShader = GLT_NULL_HANDLE;
	}

	if (_gltText2DFontTexture != GLT_NULL_HANDLE)
	{
		glad_glDeleteTextures(1, &_gltText2DFontTexture);
...
GLT_API GLboolean _gltCreateText2DShader(void)
{
	...
	glad_glShaderSource(vertexShader, 1, &_gltText2DVertexShaderSource, NULL);
	glad_glCompileShader(vertexShader);

	glad_glGetShaderiv(vertexShader, GL_COMPILE_STATUS, &compileStatus);
	...
	glad_glShaderSource(fragmentShader, 1, &_gltText2DFragmentShaderSource, NULL);
	glad_glCompileShader(fragmentShader);

	glad_glGetShaderiv(fragmentShader, GL_COMPILE_STATUS, &compileStatus);
	...
	_gltText2DShader = glad_glCreateProgram();

	glad_glAttachShader(_gltText2DShader, vertexShader);
	glad_glAttachShader(_gltText2DShader, fragmentShader);

	glad_glBindAttribLocation(_gltText2DShader, _GLT_TEXT2D_POSITION_LOCATION, "position");
	glad_glBindAttribLocation(_gltText2DShader, _GLT_TEXT2D_TEXCOORD_LOCATION, "texCoord");

	glBindFragDataLocation(_gltText2DShader, 0, "fragColor");

	glad_glLinkProgram(_gltText2DShader);

	glad_glDetachShader(_gltText2DShader, vertexShader);
	glad_glDeleteShader(vertexShader);

	glad_glDetachShader(_gltText2DShader, fragmentShader);
	glad_glDeleteShader(fragmentShader);

	glad_glGetProgramiv(_gltText2DShader, GL_LINK_STATUS, &linkStatus);
	...
	_gltText2DShaderMVPUniformLocation = glad_glGetUniformLocation(_gltText2DShader, "mvp");
	_gltText2DShaderColorUniformLocation = glad_glGetUniformLocation(_gltText2DShader, "color");

	glad_glUniform1i(glad_glGetUniformLocation(_gltText2DShader, "diffuse"), 0);
...
#undef _GLT_TEX_SET_PIXEL
	...
	glad_glGenTextures(1, &_gltText2DFontTexture);
	glad_glBindTexture(GL_TEXTURE_2D, _gltText2DFontTexture);

	glad_glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, texWidth, texHeight, 0, GL_RGBA, GL_UNSIGNED_BYTE, texData);
